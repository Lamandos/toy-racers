...
object StartupHelper {
    ...
    private const val MAC_JRE_ERR_MSG: String =
        "A Java installation could not be found. If you distribute this app with a bundled JRE, " +
            "set the '-XstartOnFirstThread' argument manually."
    private const val LINUX_JRE_ERR_MSG: String =
        "A Java installation could not be found. If you distribute this app with a bundled JRE, " +
            "set the '__GL_THREADED_OPTIMIZATIONS' environment variable to '0'."
    private const val CHILD_LOOP_ERR_MSG: String =
        "The current JVM is already a spawned child, but StartupHelper tried to spawn another one. " +
            "The game may crash or work incorrectly."
    ...
    fun startNewJvm0(
        ...
        jvmArgs += System.getenv("JAVA_MAIN_CLASS_$processID") ?: run {
            ...
            if (trace.isNotEmpty()) {
                return@run trace[trace.lastIndex].className
            } else {
