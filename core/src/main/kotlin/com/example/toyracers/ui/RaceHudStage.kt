...
class RaceHudStage(
    ...
    private val standingRows = List(MAX_VISIBLE_COMPETITORS) { index ->
        StandingRow(
            position = hudLabel("${index + 1}", 1.2f),
            name = hudLabel("RACER ${index + 1}", 1.05f, MUTED_TEXT),
        )
    }
    private val pauseButton = button("", SECONDARY_BUTTON_STYLE, onPause).apply {
        clearChildren()
    ...
    private fun createHudLayout(): Table = Table().apply {
        setFillParent(true)
        top().left().pad(22f)
    ...
    private fun createLeftColumn(): Table = Table().apply {
        top().left()
        add(neonPanel(Table().apply {
            pad(11f, 14f, 10f, 14f)
    ...
    private fun createStandings(): Table = Table().apply {
        standingRows.forEach { standing ->
            standing.root = Table().apply {
    ...
    private fun createCenterColumn(): Table = Table().apply {
        top()
    ...
    private fun createRightColumn(): Table = Table().apply {
        top().right()
    ...
    private fun neonPanel(content: Table): Stack {
        ...
        return Stack(border, Table().apply {
            pad(2f)
            add(inner).grow()
        })
    }
    ...
    private fun hudLabel(text: String, scale: Float, color: Color = Color.WHITE) =
