...
machine-readable schema for a snapshot. Every snapshot includes `schemaVersion: 1`, the simulation
...
4. Emit the schema-1 snapshot fields in the schema's array order and values, with the same
